...
let luckMessage = document.getElementById('luckMessage')
let description = document.getElementById('mainDescription')
let descriptionText1 = `Clique no biscoito da sorte para gerar a sua mensagem do dia.`
let descriptionText2 = `Caso queira gerar outra mensagem, clique no botão abaixo da frase.`
...
fortuneCookie.addEventListener('click', () => {
    randomQuote = luckQuotes[Math.floor(Math.random() * luckQuotes.length)]

    fortuneCookie.style.display = 'none'
    luckMessage.innerText = randomQuote
    luckMessage.style.display = 'block'

    if(description.innerText == descriptionText1) {
        description.innerText = descriptionText2
    }
})

resetButton.addEventListener('click', () => {
    if(fortuneCookie.style.display == 'none') {
        fortuneCookie.style.display = 'block'
    }
    if(luckMessage.style.display == 'block') {
        luckMessage.style.display = 'none'
    }

    if(description.innerText == descriptionText2) {
        description.innerText = descriptionText1
    }
})